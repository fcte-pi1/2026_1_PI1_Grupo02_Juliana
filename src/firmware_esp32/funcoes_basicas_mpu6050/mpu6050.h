...
// GYRO_CONFIG FS_SEL bits (datasheet section 4.4)
#define GYRO_FS_250DPS      0x00   // +-250 deg/s  -> 131   LSB/(deg/s)
#define GYRO_FS_500DPS      0x08   // +-500 deg/s  ->  65.5 LSB/(deg/s)
#define GYRO_FS_1000DPS     0x10   // +-1000 deg/s ->  32.8 LSB/(deg/s)
#define GYRO_FS_2000DPS     0x18   // +-2000 deg/s ->  16.4 LSB/(deg/s)

// LSB sensitivities (deg/s per count)
#define GYRO_SENS_250DPS    131.0f
#define GYRO_SENS_500DPS    65.5f
#define GYRO_SENS_1000DPS   32.8f
#define GYRO_SENS_2000DPS   16.4f

// ESP32 I2C pins (defaults for most dev boards; change to match your wiring)
#define MPU6050_SDA_PIN     21
#define MPU6050_SCL_PIN     22
#define MPU6050_I2C_FREQ    400000 // 400 kHz fast mode

// Samples used for gyroscope bias calibration
...
typedef struct {
    float x;   // bias offset in deg/s
    float y;
    float z;
} GyroBias;

typedef struct {
    float yaw;    // deg (rotation around Z-axis, used for heading)
    float pitch;  // deg (rotation around Y-axis)
    float roll;   // deg (rotation around X-axis)
} Orientation;

// Initialise I2C and the MPU-6050. Returns true on success.
bool  mpu6050_init(void);

// Read raw gyroscope data converted to deg/s.
