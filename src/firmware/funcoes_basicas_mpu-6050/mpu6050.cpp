...
bool mpu6050_init(void)
{
    ...
    // WHO_AM_I must return 0x68 (datasheet §4.28)
    uint8_t who_am_i = 0;
    ...
    // Wake up device (clear sleep bit in PWR_MGMT_1, datasheet §4.28)
    ...
    // DLPF_CFG = 3 → bandwidth 42 Hz, delay 4.8 ms (datasheet §4.3)
    if (!reg_write(REG_CONFIG, 0x03)) return false;

    // Gyroscope full scale ±250 °/s (FS_SEL = 0, datasheet §4.4)
...
void mpu6050_calibrate(GyroBias *bias)
{
    ...
    for (int i = 0; i < GYRO_CALIB_SAMPLES; i++) {
        ...
        sleep_ms(4); // ~250 Hz, well above the 125 Hz sample rate
    }
    ...
    printf("[MPU6050] Bias: X=%.4f  Y=%.4f  Z=%.4f °/s\n",
